...
import Combine
@available(OSX 10.15, *)
final class ZipTests: XCTestCase, TestCaseProtocol {
typealias Element = String
  ...
  func testZ2() -> Void {
    ...
    let z1 = s1a.zip(s1b).map(+)
    let z2 = s2a.zip(s2b).map(+)
  ...
  func testZ3() -> Void {
    ...
    let z1 = s1a.zip(s1b, s1c).map({ t in String(describing: t) })
    let z2 = s2a.zip(s2b, s2c).map({ t in String(describing: t) })
  ...
  func testZ4() -> Void {
    ...
    let z1 = s1a.zip(s1b, s1c, s1d).map({ t in String(describing: t) })
    let z2 = s2a.zip(s2b, s2c, s2d).map({ t in String(describing: t) })
